...
} from "@/features/advisory-board/actions";
import type { AdvisoryBoardMemberRow } from "@/features/advisory-board/repository";

interface Props {
  initialMembers: AdvisoryBoardMemberRow[];
}
...
export function AdvisoryBoardAdmin({ initialMembers }: Props) {
  ...
  function openEdit(member: AdvisoryBoardMemberRow) {
    setForm({
  ...
  function handleSave() {
    ...
    startTransition(async () => {
      ...
      let result;
      if (modalMode === "add") {
        result = await addAdvisoryBoardMember(data);
      } else if (modalMode === "edit" && editId) {
        result = await updateAdvisoryBoardMember(editId, data);
      }
  ...
  function handleToggleVisibility(member: AdvisoryBoardMemberRow) {
    startTransition(async () => {
      const result = await toggleAdvisoryBoardMemberVisibility(member.id, !member.is_active);
      if (result.success) {
  ...
  function handleDelete() {
    ...
    startTransition(async () => {
      const result = await deleteAdvisoryBoardMember(deleteConfirm.id, deleteConfirm.photoUrl);
      if (result.success) {
            ...
            <tbody>
              {members.map((member) => (
                <tr key={member.id} className="border-b border-gray-50 hover:bg-gray-50/40 transition-colors group">
                  ...
                  {/* Designation */}
                  <td className="px-4 py-3">
                    <span className="text-sm text-gray-600 line-clamp-2">{member.designation}</span>
                  </td>
                  {/* Order */}
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            <div className="p-6 border-b border-gray-100">
              <h2 className="text-lg font-bold text-gray-900">
                {modalMode === "add" ? "Add Advisory Board Member" : "Edit Advisory Board Member"}
              </h2>
            </div>
            <div className="p-6 space-y-5">
              ...
              <div>
                ...
                <ImageUploader folder="advisory-board" onUploadComplete={handleUploadComplete} />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#006633]/20 focus:border-[#006633]"
                  placeholder="e.g. Managing Director, ABC Corp."
                />
              </div>
              ...
              <div className="flex items-center gap-3">
                ...
                />
                <label htmlFor="ab_is_active" className="text-sm text-gray-700">Show on public page</label>
              </div>
            </div>
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm p-6">
            <h3 className="text-lg font-bold text-gray-900 mb-2">Remove Advisory Board Member</h3>
            <p className="text-sm text-gray-600 mb-6">
              Are you sure you want to remove <strong>{deleteConfirm.name}</strong> from the Advisory Board?
            </p>
