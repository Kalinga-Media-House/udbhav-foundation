...
export function AboutHeroManager({ initialImage }: AboutHeroManagerProps) {
  ...
  const handleUploadComplete = async (result: any) => {
    ...
    startTransition(async () => {
      try {
        const res = await updateSettingByKey('about_hero_background_image', `"${imageUrl}"`);
        if (res.error) {
          setError(res.error);
          return;
        }
  ...
  const handleDelete = () => {
    ...
    startTransition(async () => {
      try {
        const res = await updateSettingByKey('about_hero_background_image', `""`);
        if (res.error) {
          setError(res.error);
          return;
        }
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden mb-8">
      <div className="p-4 md:p-6 border-b border-gray-100 flex flex-col md:flex-row md:items-center justify-between gap-3 bg-gray-50/50">
        <div>
          <h2 className="text-lg font-bold text-gray-900">About Page Hero</h2>
          <p className="text-sm text-gray-500 mt-1">Manage the background image for the About page Hero section.</p>
        </div>
      </div>
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between flex-1 min-w-0 gap-2 md:gap-4 py-0.5">
                  <div className="flex flex-col min-w-0">
                    <p className="text-sm font-medium text-gray-900 truncate">
                      {image.split('/').pop() || 'about_hero_background'}
                    </p>
                  </div>
