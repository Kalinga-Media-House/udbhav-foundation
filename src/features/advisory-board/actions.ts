...
export type AdvisoryBoardActionResponse = { success: boolean; error?: string };
...
function revalidateAdvisoryBoardPaths() {
...
export async function addAdvisoryBoardMember(
  data: MemberInput
): Promise<AdvisoryBoardActionResponse> {
  try {
    const { supabase } = await requireAdminAuth();

    // Get the next display_order
    const { data: existing } = await (supabase.from('advisory_board_members' as any) as any)
    ...
    const { error } = await (supabase.from('advisory_board_members' as any) as any)
      .insert({
...
export async function deleteAdvisoryBoardMember(
  id: string,
...
export async function toggleAdvisoryBoardMemberVisibility(
  id: string,
  isActive: boolean
): Promise<AdvisoryBoardActionResponse> {
  try {
    ...
    if (error) {
      console.error('Error toggling advisory board member visibility:', error);
      return { success: false, error: error.message };
    }
...
export async function reorderAdvisoryBoardMembers(
  orderedIds: string[]
): Promise<AdvisoryBoardActionResponse> {
  try {
    const { supabase } = await requireAdminAuth();
    ...
    for (let i = 0; i < orderedIds.length; i++) {
      await (supabase.from('advisory_board_members' as any) as any)
